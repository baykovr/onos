...
public interface Database extends DatabaseProxy<String, byte[]>, Resource<Database> {
  ...
  /**
   * Creates a new storage with the default cluster configuration.<p>
   *
   * The storage will be constructed with the default cluster configuration. The default cluster configuration
   ...
   *
   * Additionally, the storage will be constructed with an storage configuration that searches the classpath for
   * three configuration files - {@code name}, {@code storage}, {@code storage-defaults}, {@code resource}, and
   * {@code resource-defaults} - in that order. The first resource is a configuration resource with the same name
   * as the map resource. If the resource is namespaced - e.g. `databases.my-storage.conf` - then resource
  ...
  /**
   * Creates a new storage.<p>
   *
   * The storage will be constructed with an storage configuration that searches the classpath for
   * three configuration files - {@code name}, {@code storage}, {@code storage-defaults}, {@code resource}, and
   * {@code resource-defaults} - in that order. The first resource is a configuration resource with the same name
   * as the storage resource. If the resource is namespaced - e.g. `databases.my-storage.conf` - then resource
   * configurations will be loaded according to namespaces as well; for example, `databases.conf`.
   *
   * @param name The storage name.
  ...
  /**
   * Tells whether the storage supports change notifications.
